...
- **`main.ts` acoplamento operacional** — main.ts concentra lifecycle, event handlers, render paths, e coordenacao entre CM6/DOM/margin. Nao e bug, e consequencia de ser o plugin entry point. Disciplina ao mexer: mudancas em event handlers ou setupDomPosition impactam todos os render paths. Preferir extrair logica pra modulos (como foi feito com domInjector, mirrorConfig, timingConfig) em vez de crescer main.ts. Anotado como nota de arquitetura, nao como item de trabalho
