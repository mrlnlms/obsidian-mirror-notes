...
- **viewOverrides em mirrors preview-only** — gap real mas nicho. `applyViewOverrides` le do CM6 StateField (criado com viewMode=source). Para mirrors LP+RV ou LP-only, funciona (StateField tem config, CSS class persiste em RV). Gap: mirrors preview-only (sem LP) → StateField tem config null → overrides nao aplicam. Cenario extremamente raro. Fix futuro: `applyViewOverrides` consultar `getApplicableConfig` com viewMode quando StateField nao tem config
